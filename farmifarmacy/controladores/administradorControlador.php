	...
	class administradorControlador extends mainModel{
        ...
        public function paginador_administrador_controlador($pagina,$registros,$url,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM usuario WHERE ((usuario_id!='$id' AND usuario_id!='".$_SESSION['id_farmifarmacy']."') AND (usuario_nombre LIKE '%$busqueda%' OR usuario_apellido LIKE '%$busqueda%' OR usuario_cargo LIKE '%$busqueda%' OR usuario_usuario LIKE '%$busqueda%')) ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";
			}else{
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM usuario WHERE usuario_id!='".$_SESSION['id_farmifarmacy']."' AND usuario_id!='1' ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";
			}
		...
		public function actualizar_administrador_controlador(){
			...
			if($id==1 && $_SESSION['id_farmifarmacy']!=$id){
				$alerta=[
                    "Alerta"=>"simple",
                    "Titulo"=>"Acceso no permitido",
                    "Texto"=>"No tienes los permisos necesarios para realizar esta operación en el sistema",
			...
			if($_SESSION['id_farmifarmacy']!=$id){
			...
			if(mainModel::actualizar_datos("usuario",$datos_usuario_up,$condicion)){
				...
				if($_SESSION['id_farmifarmacy']==$id){
					$_SESSION['nombre_farmifarmacy']=$nombre;
					$_SESSION['apellido_farmifarmacy']=$apellido;
					$_SESSION['genero_farmifarmacy']=$genero;
					$_SESSION['usuario_farmifarmacy']=$usuario;
					$_SESSION['cargo_farmifarmacy']=$cargo;
					$_SESSION['foto_farmifarmacy']=$avatar;
				}
